/**
 * Build src/data/demo-guest-menu-fr.json from demo-guest-menu-en.json.
 * Uses MyMemory public API (langpair en|fr). Run: node scripts/generate-demo-guest-menu-fr.mjs
 */
import fs from "fs";
...
const outPath = path.join(root, "src/data/demo-guest-menu-fr.json");
...
async function translate(text) {
  ...
  const u = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(t.slice(0, 450))}&langpair=en|fr`;
...
const out = en.map((cat) => ({
  ...
  categoryFr: cache.get((cat.categoryEn || cat.category).trim()) ?? cat.category,
  items: cat.items.map((it) => ({
    name: it.name,
    nameFr: cache.get((it.nameEn || it.name).trim()) ?? it.name,
    descriptionFr: (() => {
      const d = (it.descriptionEn || "").trim();
