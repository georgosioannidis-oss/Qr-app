/**
 * Build src/data/demo-guest-menu-pl.json from demo-guest-menu-en.json.
 * Uses MyMemory public API (langpair en|pl). Run: node scripts/generate-demo-guest-menu-pl.mjs
 */
import fs from "fs";
...
const outPath = path.join(root, "src/data/demo-guest-menu-pl.json");
...
async function translate(text) {
  ...
  const u = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(t.slice(0, 450))}&langpair=en|pl`;
...
const out = en.map((cat) => ({
  ...
  categoryPl: cache.get((cat.categoryEn || cat.category).trim()) ?? cat.category,
  items: cat.items.map((it) => ({
    name: it.name,
    namePl: cache.get((it.nameEn || it.name).trim()) ?? it.name,
    descriptionPl: (() => {
      const d = (it.descriptionEn || "").trim();
