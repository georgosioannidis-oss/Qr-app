...
import { parseTenantDashboardPath, tenantDashboardHref } from "@/lib/dashboard-tenant-paths";
...
export default async function TenantDashboardLayout({
  ...
  if (pathname && parsed) {
    ...
    const sessionSlug = (session?.user as { restaurantSlug?: string } | undefined)?.restaurantSlug;

    if (!session || !userId || !restaurantId) {
      redirect(`/dashboard/login?callbackUrl=${encodeURIComponent(pathname)}`);
    }

    if (sessionSlug && sessionSlug !== paramSlug) {
      redirect(tenantDashboardHref(sessionSlug, parsed.rest || ""));
    }
